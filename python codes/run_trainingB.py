...
def main():
    ...
    dataset_train, dataset_valid = generate_pipeline_training(root_folder, batch_size=GLOBAL_BATCH_SIZE)
    dist_dataset_train = strategy.experimental_distribute_dataset(dataset_train)
    dist_dataset_valid = strategy.experimental_distribute_dataset(dataset_valid)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for (x_target, y_target) in dist_dataset_train:
            
            gen_loss, disc_loss = distributed_train_step(x_target, y_target, generator, discriminator, generator_loss, discriminator_loss, generator_optimizer, discriminator_optimizer, strategy)
            
            train_gen_loss.update_state(gen_loss.numpy())
            train_disc_loss.update_state(disc_loss.numpy())
            
            counter += 1

            #print(counter)

        for (x_target, y_target) in dist_dataset_valid:

            gen_loss, disc_loss = distributed_valid_step(x_target, y_target, generator, discriminator, generator_loss, discriminator_loss, strategy)
                    
            valid_gen_loss.update_state(gen_loss.numpy())
            valid_disc_loss.update_state(disc_loss.numpy())
        ...
        with open(f'{log_folder}log_{model_name}.log','a') as fd:
            ...
            fd.write(f"{epoch},{train_gen_loss.result().numpy()},{train_disc_loss.result().numpy()},{valid_gen_loss.result().numpy()},{valid_disc_loss.result().numpy()},{end_time - start_time}\n")
...
class GAN3D(object):
    ...
    def build_architecture(self):
        ...
        if (self.model_name == 'architecture-B01'):

            return self.architecture01()

        if (self.model_name == 'architecture-B02'):

            return self.architecture02()

        if (self.model_name == 'architecture-B03'):

            return self.architecture03()

        if (self.model_name == 'architecture-B04'):

            return self.architecture04()
...
def generate_pipeline_training(root_folder, validation_split=0.2, shuffle_buffer=200, batch_size=4, n_prefetch=8):
...
@tf.function
def distributed_train_step(x_target, y_target, generator, discriminator, generator_loss, discriminator_loss, generator_optimizer, discriminator_optimizer, strategy):

    per_replica_losses_generator, per_replica_losses_discriminator = strategy.run(
        train_step, 
        args=(x_target, y_target, generator, discriminator, generator_loss, discriminator_loss, generator_optimizer, discriminator_optimizer)
    )

    loss_gen = strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses_generator, axis=None)
    loss_disc = strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses_discriminator, axis=None)
...
@tf.function
def distributed_valid_step(x_target, y_target, generator, discriminator, generator_loss, discriminator_loss, strategy):
    per_replica_losses_generator, per_replica_losses_discriminator = strategy.run(
        valid_step, 
        args=(x_target, y_target, generator, discriminator, generator_loss, discriminator_loss)
    )
    return strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses_generator, axis=None), strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses_discriminator, axis=None)
