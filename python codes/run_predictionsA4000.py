...
def main():
    ...
    ckpt_file = f"{root_folder}/models/checkpoints_{model_name}/ckpt-15"
    ...
    np.save('/  /y_targetA03_4000.npy', y_target)
    np.save('/  /y_predicA03_4000.npy', y_predic)
    np.save('/  /x_targetA03_4000.npy', x_target)
    
    errors = {'y': Y[1:ny], 'error_u': error_u[1:ny], 'error_v': error_v[1:ny], 'error_w': error_w[1:ny]}
    savemat('error_dataA03_4000.mat', errors)
...
class GAN3D(object):
    ...
    def architecture01(self):
        ...
        def generator_loss(fake_y, y_predic, y_target):
            ...
            Ycoords = np.load('coordY.npy')
            #print('\n Y coordinates: \n', Ycoords.shape, Ycoords)
            Yweights = np.empty(64, dtype = np.float32)
            Yweights[0] = (Ycoords[1] + Ycoords[0]) /2
            for i in range(1,Yweights.size):
                Yweights[i] = (  Ycoords[i+1] - Ycoords[i-1] ) /2
            #print('\n Y weights: \n', Yweights.shape, '\n Y: ', Yweights)
            #print(y_target.shape)
            Wsqrt = np.empty([y_target.shape[0], y_target.shape[1], y_target.shape[2], y_target.shape[3], y_target.shape[4]], dtype = np.float32)
            
            for i in range(Yweights.size):
                Wsqrt[:,:,i,:,:] = np.sqrt(Yweights[i])
            
            content_loss = tf.keras.losses.MSE(Wsqrt * y_target, Wsqrt * y_predic) / np.sum(Yweights) * (y_target.shape[1])#*y_target.shape[2]*y_target.shape[3])
            '''
